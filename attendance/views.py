...
def student_dashboard_view(request):
    ...
    available_teachers = User.objects.filter(role=User.TEACHER).exclude(subject="")
    ...
    if request.method == "POST":
        form = AttendanceMarkForm(request.POST, available_teachers=available_teachers)
        if form.is_valid():
            teacher_id = int(form.cleaned_data["teacher"])
